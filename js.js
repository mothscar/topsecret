...
const roomName = '🔥⚽|🏆[SPHB REVIVAL-PUB]🏆 | v3/v4 ⚽🔥';
const maxPlayers = 28;
const roomPublic = headless ? false : true;
const token = ''; // Insert token here
const roomGeo = { 'code': 'us', 'lat': 37, 'lon': -95 };
...
let roomWebhook = 'https://discord.com/api/webhooks/1263220277882388530/el0jNnpT59iuh_PHRQNGut4dYRi5tlt5d1HzBu8AFnc3IMW_XhTNs1sRMcCFSrgT3TnY'; // this webhook is used to send the details of the room (chat, join, leave) ; it should be in a private discord channel
let gameWebhook = 'https://discord.com/api/webhooks/1263220211713310811/SpdHPd68VNTl9BzjXDj_Sm6axe0Yl6EjxjTRn3DT3cm7V5qcBn4QIvjC5Po-kZAIATsE'; // this webhook is used to send the summary of the games ; it should be in a public discord channel
...
const classicMap = `{
   "name":"SPHB Beta Classic from HaxMaps",
   "width":420,
   "height":200,
   "spawnDistance":170,
   "bg":{
...
let masterList = [
    'X1HMnxn0pIsnQpAyLj6oe4PcONOPBXf5Q0A_Wn6TWbQ', //TAW
    'wVplzG6sqDvhzYpDvTgr1Xqj1MTRRhR8CzAgsnosJKs', //LeStorm
    'DBCSxwLnoX1U3OL1L7sD8X08j0BfyWlsoyUZRq22pRY', //comrade
];
let mvpList = []
...
let plaeyrsLimitPassowrd = 'ogiweh2892ioshioh9219'
...
Welcome to SPHB Revival Pub By metro boomin and comrade    
...
room.onPlayerChat = function (player, message) { //chat
    ...
    if (authArray[player.id][0] == 'wVplzG6sqDvhzYpDvTgr1Xqj1MTRRhR8CzAgsnosJKs') {
        room.sendAnnouncement(`🏆 [sphb| Lvl ${playerLevel}] ${player.name}: ${message}`, undefined, 0xFFD000, 'bold', 0); //sphbstorm
    } else if (authArray[player.id][0] == 'X1HMnxn0pIsnQpAyLj6oe4PcONOPBXf5Q0A_Wn6TWbQ') {
        room.sendAnnouncement(`💀 [TAW | Lvl ${playerLevel}] ${player.name}: ${message}`, undefined, 0x8A8784, 'normal', 0); //TAW
    } else if (authArray[player.id][0] == 'DBCSxwLnoX1U3OL1L7sD8X08j0BfyWlsoyUZRq22pRY') {
        room.sendAnnouncement(`🗣️ [comrade | Lvl ${playerLevel}] ${player.name}: ${message}`, undefined, 0x174A52, 'normal', 0); //player name
    }
...
function printDiscord(player, message) {
    room.sendAnnouncement('💫 Want to join or league or just discuss with the community? Join the discord server using this link!\nhttps://discord.gg/YrybMVyB', player.id, announcementColour, 'bold')
}

class Notify {
    discord() {
        room.sendAnnouncement(`🚨 Join our discord server here: https://discord.gg/YrybMVyBma!`, null, announcementColour, "bold", null);
    }
    coins() {
